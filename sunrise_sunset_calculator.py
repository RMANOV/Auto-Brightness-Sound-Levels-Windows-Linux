...
Provides optimized time windows (90min before, 6h after) around sunrise and sunset for intelligent scheduling
"""
...
class SunCalculator:
    ...
    def _hour_angle_sunrise(self, latitude, solar_dec):
        ...
        try:
            ha_arg = math.cos(self._radians(90.833)) / (math.cos(lat_rad) * math.cos(sdec_rad)) - math.tan(
                lat_rad
            ) * math.tan(sdec_rad)
    ...
    def get_activation_windows(self, date=None):
        """
        Get sunrise and sunset activation windows (90 min before, 6 hours after)
        ...
        return {
            ...
            "sunrise_end": add_hours_to_time(sun_times["sunrise"], 6.0),
            "sunset_start": add_hours_to_time(sun_times["sunset"], -1.5),
            "sunset_end": add_hours_to_time(sun_times["sunset"], 6.0),
        }
...
def main():
    ...
    try:
        ...
        if args.check_active:
            ...
            print(f"  Sunset: {times['sunset']}")
            print()
            print("Activation Windows (90min before, 6h after):")
