...
def main():
    ...
    avg_brightness = benchmark_function(
        controller.calculate_brightness, test_frame, iterations=5000, name="calculate_brightness (JIT)"
    )

    # Audio noise level calculation
    avg_audio = benchmark_function(
        controller._compute_noise_level_jit, test_audio, iterations=5000, name="compute_noise_level (JIT)"
    )
    ...
    avg_smooth = benchmark_function(
        controller._smooth_transition_jit, 25.0, 30.0, 0.3, iterations=10000, name="smooth_transition (JIT)"
    )

    # Screen brightness analysis
    benchmark_function(
        controller._analyze_screen_brightness_jit, test_img, iterations=1000, name="screen_brightness_analysis (JIT)"
    )
    ...
    print("Main loop efficiency target  : <500ms update interval")
